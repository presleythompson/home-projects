...
- **Sort & filter** (labeled toggles, "SORT BY" / "FILTER BY"): `home-tasks:view` sorts "By project" ⇄ "By person" (`PeopleView` groups open tasks per person + an Unassigned bucket; rows carry a project tag). `home-tasks:filter` toggles "All tasks" ⇄ "My tasks" (only tasks assigned to the current person — in project view also drops projects you have no tasks in; in person view shows only your own section).
